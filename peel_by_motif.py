...
def node_chisquare(edgelist, node_num, dist, adjust_idx=1):
    '''
...
def init_heap_dict_weighted(edgelist):
    ...
    #     G.add_edge(edge[0], edge[1], weight=edge[2])
        n = edge[0]
        if n not in node_d:
            node_d[n] = (Node(n, motif_types), fibheap.insert(edge[2], n))
        else:
            fibheap.decrease_key(node_d[n][1], node_d[n][1].key+edge[2])
...
def several_peel_weighted(edges, node_chisquares, peel_times=3, log=True):
    pairs = []
    removed_nodes = []
    for edge in edges:
